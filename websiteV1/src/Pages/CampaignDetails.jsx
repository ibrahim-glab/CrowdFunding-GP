...
function CampaignDetails() {
  ...
  const remainingDays = daysLeft(state.campaignEndTime);
  // Calling Contribute Function
  const { contract } = useContract(
    import.meta.env.VITE_CONTRACTADDRESS,
    contractABI
  );
  const { mutateAsync } = useContractWrite(contract, "contribute");
  ...
  // handle the onClick of fund button
  const handleFund = async () => { // Convert ETH amount to wei
      setIsLoading(true); 
      if (data5){
        data5.map((campaign) => {
          if(campaign.data.campaign === state.address){
            campaign.data.totalContributions += parseFloat(amount);
          }
        })
      }
      await mutateAsync({ args: [state.address] });
      setIsLoading(false);
  };
