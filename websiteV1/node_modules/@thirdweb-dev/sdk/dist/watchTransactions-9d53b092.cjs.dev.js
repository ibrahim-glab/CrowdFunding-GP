...
async function getContractFromAbi(params) {
  ...
  const [resolvedAddress, {
    ...
    SmartContract
  }] = await Promise.all([index.resolveAddress(params.address), provider.getNetwork(), Promise.resolve().then(function () { return require('./smart-contract-1634dbbc.cjs.dev.js'); })]);
...
async function getContract(params) {
  ...
  const [resolvedAddress, {
    ...
    ContractPublisher
  }] = await Promise.all([index.resolveAddress(params.address), provider.getNetwork(), Promise.resolve().then(function () { return require('./contract-publisher-86caec67.cjs.dev.js'); }).then(function (n) { return n.contractPublisher; })]);
  if (inContractCache(resolvedAddress, chainId)) {
    return getCachedContract(resolvedAddress, chainId);
  }
