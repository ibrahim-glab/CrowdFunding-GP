...
const checkForSdkUpdates = async () => {
  try {
    const {
      response
    } = await _RESTController.default.ajax('GET', 'https://www.unpkg.com/moralis-v1/package.json');
    const latestVersion = response.version;
    if (isSemanticVersionLarger(latestVersion, "1.12.0")) // eslint-disable-next-line no-console
      console.warn(`Moralis: You are not using the latest version of the 'moralis-v1' SDK. Please update it as soon as possible to enjoy the newest features. Most recent version: ${latestVersion}`);
